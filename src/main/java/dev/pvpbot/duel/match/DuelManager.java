...
public final class DuelManager implements Listener {
    ...
    @EventHandler(priority=EventPriority.MONITOR,ignoreCancelled=true) public void knockback(EntityKnockbackEvent e){DuelMatch m=findByEntity(e.getEntity().getUniqueId());if(m==null||m.state()!=MatchState.ACTIVE||m.bot()==null||m.brain()==null)return;Player bot=m.bot().entity();if(bot==null||!e.getEntity().getUniqueId().equals(bot.getUniqueId()))return;org.bukkit.util.Vector impulse=e.getKnockback();if(Math.hypot(impulse.getX(),impulse.getZ())>1.0E-9)m.brain().incomingKnockback(e.getCause());}
