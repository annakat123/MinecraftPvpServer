...
public final class DuelManager implements Listener {
    ...
    private final Map<UUID,DuelMatch> matches=new HashMap<>(); private final Map<UUID,Difficulty> selected=new HashMap<>(); private final Map<UUID,BotProfile> custom=new HashMap<>();
    ...
    public void start(Player p){DuelMatch existing=matches.get(p.getUniqueId());if(existing!=null&&existing.state()==MatchState.FINISHED)matches.remove(p.getUniqueId());else if(existing!=null){p.sendMessage("§cYou are already in a duel.");return;}Optional<Arena> allocated=arenas.allocate();if(allocated.isEmpty()){p.sendMessage("§cAll arenas are busy.");return;}DuelMatch match=new DuelMatch(facade(),p,selectedProfile(p),allocated.get(),arenas,lobby,bots,mode.kit(),database);matches.put(p.getUniqueId(),match);try{match.start();}catch(RuntimeException e){matches.remove(p.getUniqueId());p.sendMessage("§cDuel could not start. See server log.");}}
    ...
    public void cleanup(Player p){DuelMatch m=matches.remove(p.getUniqueId());if(m!=null)m.cleanup();}
