...
public final class DuelManager implements Listener {
    ...
    public void start(Player p){UUID playerId=p.getUniqueId();if(matches.containsKey(playerId)){p.sendMessage("§cYou are already in a duel.");return;}Optional<Arena> allocated=arenas.allocate();if(allocated.isEmpty()){p.sendMessage("§cAll arenas are busy.");return;}DuelMatch match=new DuelMatch(facade(),p,selectedProfile(p),allocated.get(),arenas,lobby,bots,mode.kit(),database,closed->matches.remove(playerId,closed));matches.put(playerId,match);try{match.start();}catch(RuntimeException e){matches.remove(playerId,match);p.sendMessage("§cDuel could not start. See server log.");}}
    ...
    @EventHandler public void quit(PlayerQuitEvent e){Player p=e.getPlayer();cleanup(p);selected.remove(p.getUniqueId());custom.remove(p.getUniqueId());}
