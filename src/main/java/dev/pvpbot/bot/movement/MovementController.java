...
public final class MovementController {
    private final Random random=new Random(); private int strafe=1; private long nextSwitch; private int sprintPauseTicks;
    public void tick(Player bot,Player target,Arena arena,BotProfile p,Decision decision,long tick,int incomingCombo){
        Vector toward=target.getLocation().toVector().subtract(bot.getLocation().toVector()); toward.setY(0); double distance=toward.length(); if(distance>.001)toward.normalize();
        if(tick>=nextSwitch){strafe=random.nextBoolean()?1:-1;double skill=p.value("strafe.skill");nextSwitch=tick+6+random.nextInt((int)(18-8*skill));}
        ...
        double side=p.enabled("strafe")&&random.nextDouble()<p.value("strafe.chance") ? .10+.11*p.value("strafe.intensity") : 0;
        ...
        move.setY(bot.getVelocity().getY()); bot.setVelocity(move); if(sprintPauseTicks>0){bot.setSprinting(false);sprintPauseTicks--;}else bot.setSprinting(true);
