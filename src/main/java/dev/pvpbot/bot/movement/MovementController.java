...
public final class MovementController {
    ...
    public void tick(Player bot,Location targetLocation,Arena arena,BotProfile p,Decision decision,long tick,int incomingCombo){
        ...
        if(incomingCombo>0&&p.enabled("jumpReset")&&bot.isOnGround()&&techniqueRandom.nextDouble()<p.value("jumpReset.chance")*p.value("jumpReset.skill")*.14)bot.setVelocity(bot.getVelocity().setY(.42));
    }
