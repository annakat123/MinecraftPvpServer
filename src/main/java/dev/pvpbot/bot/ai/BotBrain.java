...
public final class BotBrain {
    ...
    public BotBrain(BotHandle handle,Player target,Arena arena,BotProfile profile,Telemetry telemetry){this.handle=handle;this.target=target;this.arena=arena;this.profile=profile;this.telemetry=telemetry;previousDistance=handle.entity().getLocation().distance(target.getLocation());}
    public void tick(ComboTracker combo){tick++;combo.expire(org.bukkit.Bukkit.getCurrentTick(),25);Player bot=handle.entity();if(bot==null||!target.isOnline())return;double distance=bot.getLocation().distance(target.getLocation()),closing=previousDistance-distance;previousDistance=distance; Vector rel=target.getVelocity();adaptation.model().observe(closing,rel.getX()+rel.getZ(),!target.isOnGround());
        boolean grounded=grounded(bot);PerceptionSnapshot raw=new PerceptionSnapshot(tick,distance,closing,target.getVelocity().getY(),bot.getVelocity().getY(),bot.getHealth(),target.getHealth(),combo.playerCombo(),combo.botCombo(),tick-lastIncoming,tick-lastOutgoing,bot.hasLineOfSight(target),grounded,target.isOnGround());long now=System.currentTimeMillis();latency.offer(now,profile.millis("simulatedPingMs"),raw);var perceived=latency.poll(now);if(perceived.isEmpty())return;
        ...
        aimEligible=aim.aim(bot,target,profile,adaptation.aimLateralBias(profile));if(grounded&&tick-lastIncoming>6)movement.tick(bot,target,arena,profile,decision,tick,combo.playerCombo());
        if(decision==Decision.CRITICAL_ATTACK){if(bot.isOnGround()&&critical.tryStart(bot,profile)){return;}if(Math.abs(bot.getVelocity().getY())>.02&&!critical.criticalWindow(bot))return;} double reach=profile.enabled("reach")?profile.value("reach.blocks"):3.0;boolean attackWatchdog=tick-lastAttack>=30&&perceived.get().distance()<=reach&&perceived.get().lineOfSight();if((isAttack(decision)||attackWatchdog)&&tick-lastAttack>=10&&perceived.get().distance()<=reach&&perceived.get().lineOfSight()&&(aimEligible||attackWatchdog)){boolean crit=critical.criticalWindow(bot);bot.attack(target);bot.swingMainHand();lastAttack=tick;lastOutgoing=tick;movement.afterAttack(bot,profile);telemetry.botAttackAttempt(true,crit);}
    }
