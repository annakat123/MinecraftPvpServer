...
public final class BotBrain {
    ...
    public void tick(ComboTracker combo){tick++;Player bot=handle.entity();if(bot==null||!target.isOnline())return;double distance=bot.getLocation().distance(target.getLocation()),closing=previousDistance-distance;previousDistance=distance; Vector rel=target.getVelocity();adaptation.model().observe(closing,rel.getX()+rel.getZ(),!target.isOnGround());
        ...
        aimEligible=aim.aim(bot,target,profile,adaptation.aimLateralBias(profile));if(bot.isOnGround()&&tick-lastIncoming>6)movement.tick(bot,target,arena,profile,decision,tick,combo.playerCombo());
