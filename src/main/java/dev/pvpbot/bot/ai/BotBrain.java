...
public final class BotBrain {
    ...
    public void tick(ComboTracker combo){tick++;if(!bot.isValid()||!target.isOnline())return;double distance=bot.getLocation().distance(target.getLocation()),closing=previousDistance-distance;previousDistance=distance; Vector rel=target.getVelocity();adaptation.model().observe(closing,rel.getX()+rel.getZ(),!target.isOnGround());
        ...
        aimEligible=aim.aim(bot,target,profile,adaptation.aimLateralBias(profile));movement.tick(bot,target,arena,profile,decision,tick,combo.playerCombo());
