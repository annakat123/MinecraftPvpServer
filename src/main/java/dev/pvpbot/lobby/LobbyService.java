...
public final class LobbyService {
    ...
    public void send(Player p) { p.teleport(spawn); p.setGameMode(GameMode.ADVENTURE); p.setHealth(20); p.setFoodLevel(20); p.setSaturation(20); p.setExp(0); p.setLevel(0); p.getInventory().clear(); p.getInventory().setArmorContents(null); p.getInventory().setItem(0,item(Material.DIAMOND_SWORD,"§bDuel Selector")); p.getInventory().setItem(4,item(Material.COMPARATOR,"§eBot Settings")); p.getInventory().setItem(8,item(Material.WRITTEN_BOOK,"§aStatistics")); }
    private ItemStack item(Material material,String name) { ItemStack i=new ItemStack(material); ItemMeta m=i.getItemMeta(); m.setDisplayName(name); i.setItemMeta(m); return i; }
}
