...
- **Jump Reset:** one confirmed player -> bot `EntityDamageByEntityEvent` at tick T creates one opportunity and samples the deterministic `TECHNIQUE` RNG exactly once. It cannot execute inside that damage callback. A selected opportunity may execute one normal `Y=0.42` player jump at T+1 or, if not grounded then, at T+2; before T+1 it waits and at T+3 it has expired. Combo state, critical setup, WHIFFs and AttackIntents cannot create, consume or retry it. Current X/Z is preserved, including while the knockback lock suppresses normal movement writes. Whether this timing actually reduces knockback on Paper/Citizens 26.2 remains a mandatory client-physics test.
- **Critical setup:** remains proactive and independent of incoming hits, but now uses the normal `Y=0.42` player jump and preserves X/Z. Paper exposes `setJumping`, but Citizens 2.0.43 `EntityHumanNPC` consumes that state into `jumpFromGround()` only while Citizens navigation is active; PvPBot intentionally does not use Citizens navigation, so the public velocity boundary is used.
- **Incoming knockback:** PvPBot observes Paper's uncancelled public `EntityKnockbackEvent` and accepts a non-zero horizontal impulse only when the authoritative player -> bot damage event occurred on the same `BotBrain` tick and the Paper cause is `ENTITY_ATTACK` or `SWEEP_ATTACK`. Paper 26.2 source fires the damage event during accepted-damage calculation and then the melee knockback event later in the same `hurtServer` call. `PUSH`, `EXPLOSION`, `DAMAGE`, `UNKNOWN` and `SHIELD_BLOCK` cannot open the lock. A valid impulse starts a four-tick X/Z lock on the following AI tick. Approach/retreat, strafe and S-tap writes are suppressed while Minecraft friction, gravity and velocity remain authoritative; catastrophic arena-boundary recovery stays allowed. The held movement plan resumes after the lock. `simulatedPingMs` never delays this physical signal.
- **Attack animation:** `AttackExecutor` requests one animation before its physical probe. `CitizensBotAnimation` calls `PlayerAnimation.ARM_SWING.play(bot, duelPlayer)`; Citizens `v26_2_R1` emits one `ClientboundAnimatePacket` with animation code 0. PvPBot sends no additional explicit swing around `bot.attack(target)`.
- **Vertical debug:** `/pvpbot debug` reports the last vertical cause as `vertical=NONE|CRIT|JRESET|KB`, `kbLock`, the short NPC entity UUID plus reference revision, and `npcSpawned`. Only a `CRITICAL_SETUP`-origin falling window marks a planned critical; JRESET airborne state alone does not.

The 1.0.10 runtime pipeline is world state → one immutable `PerceptionSnapshot` → one logical simulated-latency buffer → latest matured/perceived snapshot. A new matured snapshot is learned by adaptation exactly once. Three independent scheduled gates then control decision, aim planning, and movement planning; held state is executed between gate openings. The implementation never sleeps, delays packets, or creates one scheduler task per reaction.
...
Each immutable `AttackIntent` records a monotonic sequence, creation tick, source perception tick, held decision, `DECISION`/`WATCHDOG` source, perceived distance/LOS, configured reach and intended-critical flag. It is consumed exactly once on the same `BotBrain` tick when ready; 1.0.10 adds no generic click or motor delay. Consumption records one ACTIVE bot attempt, requests one packet-backed Citizens attack animation, then performs a neutral current-world ray trace from the bot eye along its physically executed view direction. The combined Paper `World.rayTrace(Location, Vector, double, FluidCollisionMode, boolean, double, Predicate)` call uses configured reach, precise block collision shapes, the closest entity hit and `raySize=0`. Solid obstruction, another entity, current aim error or a moved/out-of-reach target therefore produces `WHIFF`; invalid/despawned/cross-world targets produce `TARGET_INVALID`. Only a ray result whose entity is the duel target calls `bot.attack(target)`.
